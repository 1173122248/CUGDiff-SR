...
class RealESRGANDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            with open(self.opt['meta_info_file'], encoding='utf-8') as fin:
                paths = [line.strip().split(' ')[0] for line in fin]
                self.paths = [os.path.join(self.gt_folder, v) for v in paths]
        else:
            if isinstance(self.gt_folder, list) or isinstance(self.gt_folder, tuple):
                self.paths = []
                for folder in self.gt_folder:
                    self.paths.extend(sorted(list(scandir(folder, full_path=True))))
            else:
